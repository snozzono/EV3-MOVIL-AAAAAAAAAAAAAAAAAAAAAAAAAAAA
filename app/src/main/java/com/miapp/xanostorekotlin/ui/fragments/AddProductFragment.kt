...
class AddProductFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.scrollContainer.setOnScrollChangeListener { _, _, scrollY, _, _ ->
            binding.fabBackToTop.visibility = if (scrollY > thresholdPx) View.VISIBLE else View.GONE
        }
        binding.fabBackToTop.setOnClickListener {
            binding.scrollContainer.smoothScrollTo(0, 0)
        }

        // Modo edición si recibimos product_id
        productIdToEdit = arguments?.getInt("product_id")?.takeIf { it > 0 }
        if (productIdToEdit != null) {
            binding.btnSubmit.text = getString(com.miapp.xanostorekotlin.R.string.edit)
    ...
    private fun submit() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                val service = RetrofitClient.createProductService(requireContext()) // Servicio de productos
                ...
                if (productIdToEdit == null) {
                    ...
                    if (resp != null) {
                        ...
                        navigateBackToAdminList()
                    } else {
                        Toast.makeText(requireContext(), "Error: No se recibió confirmación del servidor", Toast.LENGTH_LONG).show()
                    }
                } else {
                    val updateReq = com.miapp.xanostorekotlin.api.UpdateProductRequest(
                        name = name,
                        description = description,
                        price = price,
                        images = imageObjects
                    )
                    val updated = withContext(Dispatchers.IO) { service.updateProduct(productIdToEdit!!, updateReq) }
    ...
    private fun loadProductForEdit(id: Int) {
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                val product = withContext(Dispatchers.IO) { service.getProduct(id) }
                binding.etName.setText(product.name)
                binding.etDescription.setText(product.description ?: "")
                binding.etPrice.setText(product.price?.toString() ?: "")
            } catch (e: Exception) {
                Toast.makeText(requireContext(), "No se pudo cargar el producto", Toast.LENGTH_LONG).show()
            }
        }
    }

    // Navega de regreso a la lista/admin tras crear o editar
    private fun navigateBackToAdminList() {
