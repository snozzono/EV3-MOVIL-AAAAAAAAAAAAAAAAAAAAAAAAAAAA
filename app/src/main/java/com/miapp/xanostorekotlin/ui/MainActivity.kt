...
class MainActivity : AppCompatActivity() { // Activity principal de login
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // Ciclo de vida: creación de la Activity
        ...
        // Si ya hay sesión, vamos directo según el rol guardado
        if (tokenManager.isLoggedIn()) { // Consultamos si hay token guardado
            // Mensaje de bienvenida si ya estaba logueado
            ...
            navigateByRole(tokenManager.getUserRole()) // Navegamos según rol
            finish()
        ...
        binding.btnLogin.setOnClickListener { // Click en botón Login
            ...
            lifecycleScope.launch {
                try {
                    ...
                    navigateByRole(userProfile.role) // Navegamos según rol
    ...
    private fun navigateByRole(role : String?) {
        val intent = if (role?.lowercase() == "admin") {
            ...
            Intent(this, HomeClientActivity::class.java)
        }
