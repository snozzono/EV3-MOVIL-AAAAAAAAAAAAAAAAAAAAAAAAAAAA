...
---
Sugerencia pedagógica: lee primero `ApiConfig`, `RetrofitClient` y los `Service`. Luego recorre `MainActivity` → `HomeActivity` → Fragments y Adapters. Observa cómo el token fluye con `AuthInterceptor` y cómo `ViewBinding` simplifica el acceso a vistas.#   E V 3 - M O V I L - A A A A A A A A A A A A A A A A A A A A A A A A A A A A  
 